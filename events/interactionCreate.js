    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (["deleteMessage", "timeout", "kick", "ban"].includes(id[0])) {
        ...
        if (id[0] === "deleteMessage") {
          ...
          if (
            admin ||
            channel?.permissionsFor(interaction.member).has("ManageMessages")
          ) {
            if (
              channel
              ...
              if (message) {
                try {
                  ...
                  if (interaction.message.components.length === 1) {
                    ...
                    const row1 = Discord.ActionRowBuilder.from(
                      interaction.message.components[0]
                    );

                    const buttons = [];
                    for (const button of interaction.message.components[1]
                      .components) {
                      const newButton =
                        Discord.ButtonBuilder.from(button).setDisabled();
                  ...
                  await interaction.editReply({
                    content: "The message has been deleted successfully.",
                  });
                } catch (error) {
                  await interaction.editReply({
                    content: `There was an error while deleting this message, *(${error.message})*`,
                  });
                }
              } else {
                const row1 = Discord.ActionRowBuilder.from(
                ...
                await interaction.editReply({
                  content: "This message has already been deleted.",
                });
              }
            } else
              await interaction.editReply({
                content: `I need Manage Message permission in ${channel}.`,
              });
          } else
            await interaction.editReply({
              content: "You can't use this button. you are not an Admin.",
            });
          ...
          if (admin || interaction.member.permissions.has("ModerateMembers")) {
            ...
            if (member) {
                ...
                if (member.moderatable) {
                  if (
                    ...
                    return await interaction.editReply({
                      content: `You can't timeout ${member.user.tag}`,
                    });
                  ...
                  collector.on("collect", async (int) => {
                    if (int.customId === "Cancel") {
                      ...
                      embed.addFields({
                        name: "Punish Action:",
                        value: `${member.user.tag} timed out by ${
                          interaction.user
                        } \`(${interaction.user.id})\` for ${ms(duration, {
                          long: true,
                        })}.`,
                      });
                  ...
                  await interaction.editReply({
                    content: `I can't timeout ${member.user.tag}.`,
                  });
              } catch (error) {
                await interaction.editReply({
                  content: `There was an error while timeouting ${member}, *(${error.message})*`,
                });
              }
            } else {
              ...
              if (user)
                await interaction.editReply({
                  content: `${user.tag} is no longer in the server.`,
                });
              else
                await interaction.editReply({
              ...
              try {
                if (member.kickable) {
                  if (
                    ...
                    return await interaction.editReply({
                      content: `You can't kick ${member.user.tag}`,
                    });
                  ...
                  await interaction.editReply({
                    content: `I can't kick ${member.user.tag}.`,
                  });
              } catch (error) {
                await interaction.editReply({
                  content: `There was an error while kicking ${member}, *(${error.message})*`,
                });
              }
            } else {
              const user = await client.users.fetch(id[1]).catch(() => null);
          ...
          if (admin || interaction.member.permissions.has("BanMembers")) {
            ...
            if (member) {
              try {
                if (member.bannable) {
                  if (
                    ...
                    return await interaction.editReply({
                      content: `You can't ban ${member.user.tag}`,
                    });
                  ...
                  collector.on("collect", async (int) => {
                    if (int.customId === "Cancel") {
                      ...
                      const duration = ms(int.customId);
                      await member.ban({
                        deleteMessageSeconds: duration / 1000,
                      ...
                      let fieldValue = `${member.user.tag} banned by ${
                        interaction.user
                      } \`(${
                        interaction.user.id
                      })\` and all their messages in the last ${ms(duration, {
                        long: true,
                      })} were deleted.`;
                      if (duration === 0)
                        fieldValue = `${member.user.tag} banned by ${interaction.user} \`(${interaction.user.id})\``;
                  ...
                  await interaction.editReply({
                    content: `I can't ban ${member.user.tag}.`,
                  });
              } catch (error) {
                await interaction.editReply({
                  content: `There was an error while banning ${member}, *(${error.message})*`,
                });
              }
            } else {
              const user = await client.users.fetch(id[1]).catch(() => null);
              ...
              try {
                const fetchedBan = await interaction.guild.bans
                  .fetch(user.id)
                  .catch(() => null);
                if (!fetchedBan) {
                  const banButton1 = new Discord.ButtonBuilder()
                  ...
                  collector.on("collect", async (int) => {
                    if (int.customId === "Cancel") {
                      ...
                      const duration = ms(int.customId);
                      await interaction.guild.bans.create(id[1], {
                        deleteMessageSeconds: duration / 1000,
                        reason: `Banned by ${interaction.user.tag}`,
                      });
                      ...
                      let fieldValue = `${user ? user.tag : id[1]} banned by ${
                        interaction.user
                      } \`(${
                        interaction.user.id
                      })\` and all their messages in the last ${ms(duration, {
                      ...
                      if (duration === 0)
                        fieldValue = `${user ? user.tag : id[1]} banned by ${
                          interaction.user
                        } \`(${interaction.user.id})\``;
                  ...
                  await interaction.editReply({
                    content: `${fetchedBan.user.tag} has been already banned.`,
                  });
              } catch (error) {
                await interaction.editReply({
                  content: `There was an error while banning this user, *(${error.message})*`,
                });
              }
            }
          } else
