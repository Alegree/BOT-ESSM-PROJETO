...
module.exports = async (client, message) => {
  ...
  if (
    moderation.State &&
    !moderation.IgnoredChannels.includes(message.channelId) &&
    !moderation.IgnoredUsers.includes(message.author.id)
  ) {
      ...
      openai.moderations
        ...
        .then(async (response) => {
          ...
          if (data.flagged) {
            ...
            const trueFlags = Object.keys(flags.flags).filter(
              (key) => flags.flags[key]
            );

            const sameFlagsWithAutoDelete = trueFlags.filter(
              (key) => moderation.AutoDelete[key]
            );
            ...
            const sameFlagsWithAutoPunish = trueFlags.filter(
              (key) => moderation.AutoPunish[key]
            );
            ...
            if (sameFlagsWithAutoPunish.length) {
              ...
              const punishTypes = sameFlagsWithAutoPunish.map(
                (key) => moderation.AutoPunishType[key]
              );
              ...
              if (punishType === "Timeout" || punishType === "Ban") {
                const punishDurations = sameFlagsWithAutoPunish
                  .filter(
                    (key) => moderation.AutoPunishType[key] === punishType
                  )
                  .map((key) => moderation.AutoPunishDuration[key]);
                let duration;
            ...
            if (memberPunishResult.Punished) {
              let fieldValue;
              if (memberPunishResult.Action === "Timeout")
                fieldValue = `${
                  message.author.tag
                } timed out automatically for ${ms(
                  memberPunishResult.Duration,
                  { long: true }
                )}`;
              else if (memberPunishResult.Action === "Ban")
                fieldValue = `${
                  message.author.tag
                } banned automatically and all their messages in the last ${ms(
                  memberPunishResult.Duration,
                  { long: true }
                )} were deleted.`;
              else if (memberPunishResult.Action === "Kick")
                fieldValue = `${message.author.tag} kicked automatically `;
            ...
            if (buttons.includes("Message")) {
              ...
              const messageRow = new Discord.ActionRowBuilder().addComponents([
                deleteMessageButton,
                jumpButton,
              ]);
  ...
  if (
    chatbot.State &&
    chatbot.ChatBotChannel === message.channelId &&
    !chatbot.IgnoredUsers.includes(message.author.id)
  ) {
    ...
    if (conversations.has(message.author.id))
      oldMessages = conversations.get(message.author.id);
    if (oldMessages) {
    ...
    openai.chat.completions
        ...
        for await (const part of response) {
          if (part.choices[0]?.finish_reason === "stop") {
            ...
            if (conversations.has(message.author.id)) {
              ...
              conversations.set(
                message.author.id,
                oldConversation.concat(newDataArray)
              );
            } else {
      ...
      .catch(async (error) => {
        ...
        if (error.response)
          await message.reply({
            content:
              error.response.error.message.length > 4000
                ? error.response.error.message.substring(0, 3097) + "..."
          ...
          await message.reply({
            content:
              error.message.length > 4000
                ? error.message.substring(0, 3097) + "..."
                : error.message,
          });
      });
  }
